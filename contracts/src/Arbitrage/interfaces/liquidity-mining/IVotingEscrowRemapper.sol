...
interface IVotingEscrowRemapper {
    event AddressMappingUpdated(
        address indexed localUser,
        address indexed remoteUser,
        uint16 indexed chainId
    );
    event RemoteAddressMappingCleared(
        address indexed remoteUser,
        uint16 indexed chainId
    );
    event AddressDelegateUpdated(
        address indexed localUser,
     ...
     */
    function getOmniVotingEscrowAdaptor()
        external
        view
        returns (IOmniVotingEscrowAdaptor);
    ...
    function getUserPoint(
        address user
    ) external view returns (IVotingEscrow.Point memory);
     ...
     */
    function setNetworkRemappingManager(
        address localUser,
        address delegate
    ) external;
